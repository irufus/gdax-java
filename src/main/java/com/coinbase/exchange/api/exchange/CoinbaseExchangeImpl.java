...
public class CoinbaseExchangeImpl implements CoinbaseExchange {
    ...
    @Override
    public <T, R> T post(String resourcePath,  ParameterizedTypeReference<T> responseType, R jsonObject) {
        ResponseEntity<T> response = restTemplate.exchange(getBaseUrl() + resourcePath,
                HttpMethod.POST,
                securityHeaders(resourcePath, "POST", jsonObject),
                responseType);
        return response.getBody();
    }
    ...
    @Override
    public <R> HttpEntity<String> securityHeaders(String endpoint, String method, R jsonObject) {
        Gson gson = new Gson();
     ...
     * timestamp + method + requestPath + body (string concatenated)
